...
def test_get_device(client, setup_db):
    """测试获取单个设备。"""
    response = client.get("/api/v1/devices/API001")
    assert response.status_code == 200


def test_get_device_not_found(client):
    """测试获取不存在的设备。"""
    response = client.get("/api/v1/devices/NOTEXIST")
...
def test_recover_device(client, setup_db):
    """测试恢复设备。"""
    # 先隔离
    client.post("/api/v1/devices/API001/quarantine", json={"reason": "Test"})

    response = client.post("/api/v1/devices/API001/recover")
    assert response.status_code == 200


def test_sync_devices(client):
    """测试同步设备。"""
    response = client.post("/api/v1/devices/sync")
    assert response.status_code == 200
